...
if 'firstTime' not in st.session_state: #<-Sirve para saludar al usuario
...
if st.session_state.chatHistory == [] and st.session_state.firstTime == '':
    gretting = asyncio.run(generate_response("Presentate con el usuario y dale la bienvenida"))
    ...
    if message['role'] =='user':
        with st.chat_message('user'):
            if message == st.session_state.chatHistory[-1] and st.session_state.stream_last:
                ...
                st.write_stream(stream_text)
                st.session_state.stream_lasy = False
            else:
